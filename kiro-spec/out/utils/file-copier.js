...
class FileCopier {
     ...
     * 复制Claude Commands目录
     */
    async copyClaudeCommands(workspaceRoot, forceOverwrite = false) {
        ...
        try {
            ...
            if (!(await fs.pathExists(sourcePath))) {
                throw this.createError(types_1.ErrorType.TEMPLATE_NOT_FOUND, '模板文件不存在');
            }
            // 如果目标存在且不强制覆盖，则跳过
            ...
            throw this.createError(types_1.ErrorType.COPY_FAILED, `复制Claude Commands失败: ${error}`);
        }
    }
    /**
     * 复制CLAUDE配置文件
     */
    async copyClaudeConfig(workspaceRoot, language, forceOverwrite = false) {
        ...
        try {
            // 确保源文件存在
            if (!(await fs.pathExists(sourcePath))) {
                throw this.createError(types_1.ErrorType.TEMPLATE_NOT_FOUND, `语言模板文件不存在: ${sourceFileName}`);
            }
            // 如果目标存在且不强制覆盖，则跳过
            if (!forceOverwrite && (await fs.pathExists(targetPath))) {
                return;
            ...
            throw this.createError(types_1.ErrorType.COPY_FAILED, `复制CLAUDE配置失败: ${error}`);
        }
    }
    /**
     * 批量安装所有必要文件
     */
    async installAll(workspaceRoot, language, forceOverwrite = false) {
        try {
            // 复制Claude Commands目录
            ...
            throw this.createError(types_1.ErrorType.COPY_FAILED, `安装失败: ${error}`);
        }
    }
    /**
     * 清理已安装的文件
     */
    async cleanup(workspaceRoot) {
        ...
        try {
            ...
            throw this.createError(types_1.ErrorType.COPY_FAILED, `清理失败: ${error}`);
        }
    }
    /**
     * 创建扩展错误对象
     ...
     * 获取错误恢复建议
     */
    getRecoveryAdvice(errorType) {
        switch (errorType) {
            case types_1.ErrorType.WORKSPACE_NOT_FOUND:
                return '请先打开一个项目文件夹';
            case types_1.ErrorType.FILE_PERMISSION_DENIED:
                return '请检查文件权限或以管理员身份运行VS Code';
            case types_1.ErrorType.TEMPLATE_NOT_FOUND:
                return '请重新安装扩展或联系开发者';
            case types_1.ErrorType.COPY_FAILED:
